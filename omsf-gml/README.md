...
Feature of interest of the described Observation is represented as separate parts for the sampling feature and the ultimate feature of interest as suggested by [W3C Extensions to the Semantic Sensor Network Ontology proposal](https://w3c.github.io/sdw/proposals/ssn-extensions/).  
As the simple features profile data models for level SF-0 does not allow for including the structural (object oriented) presentation of the feature of interest of the Observation, the sampling feature is encoded by one mandatory property ```geometry``` and two optional properties ```samplingFeatureName``` and ```samplingFeatureReference```. The ```samplingFeatureReference``` property is used to optionally refer to a full, external description of the sampling feature used, if such description can be pointed to. In cases where no external reference to the sampled feature can be given, the ```samplingFeatureName``` may still be used for naming the sampling feature used. In a similar fashion the ultimate feature of interest (the model of the observed real-world object) is encoded using two optional properties: ```ultimateFeatureOfInterestName``` and ```ultimateFeatureOfInterestReference```. The ```geometry``` property of a OMSF Observation feature is the geometry of the sampling feature of the observation, or if no sampling feature was used, a representative geometry of the ultimate feature of interest.

The OMSF encoding does not specify the content or the format of the remotely referred properties in addition to what is specified for the used GML type ```gml:ReferenceType```.

Temporal properties with possible period content (phenomenonTime and validTime) have been encoded as two separate properties, one for the period start time (inclusive) and another for the period end time. The values are always of type ```xsd:dateTime```. As the ```phenomenonTime``` may also be a time instant, the O&M simple features may either include a single ```phenomenonTime``` property (instant) or one or both of ```phenomenonTimeStart``` and ```phenomenonTimeEnd``` properties. If either the start or the end is not provided, the time period must be interpreted as an open-ended period.
